...
function main() {
  ...
  writeBaseReadme(baseDir);
}

function writeBaseReadme(baseDir) {
  const readmePath = path.join(baseDir, 'README.md');
  const content = `# Mother's Day sites by country

Each subfolder (ES, FR, DE, IT, MX, BR, CA, AU) is a static site ready for build and deploy.

- **Create/update sites:** From the template repo run \`node scripts/create-country-sites.js --base="<path-to-this-folder>"\`.
- **Cities:** Edit \`data/campaigns/mothers-day-<country>.json\` in each project, then run \`node scripts/generate-city-pages.js --campaign=mothers-day-<country>\` in that folder.
- **Open in Cursor:** From the template repo run \`./scripts/open-cursor-workspaces.ps1 -Base "<path-to-this-folder>"\`.
- **Full docs:** See the template repo \`docs/MULTI-COUNTRY-SETUP.md\`.
`;
